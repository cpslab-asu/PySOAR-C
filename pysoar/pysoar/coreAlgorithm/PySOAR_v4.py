...
##### v4 ########### fixed history['sample'] and history['rob'] to be of len B
def PySOAR(inpRanges, prob, repNum, local_search):
    ...
    while sim_count < nSamples:
        ...
        print(sim_count)
    print(history['samples'].shape, history['rob'].shape, history['f_star'].shape)
    print('SOAR_Taliro: Samples Exhausted!')
    run['nTests'] = nSamples
    return run, history
