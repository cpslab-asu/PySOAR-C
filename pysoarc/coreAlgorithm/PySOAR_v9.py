...
from ..utils import calculate_robustness, EIcalc_kd, neg_EIcalc_kd, neg_CrowdingDist_kd
...
def PySOAR(n_0, nSamples, inpRanges, alpha_lvl_set, eta0, eta1, delta, gamma, eps_tr, prob, seed, local_search, folder_name, benchmark_name):
    ...
    while sim_count < nSamples and run['falsified'] != 1:
        ...
        TR_Bounds = np.vstack(
            [x0 - inpRanges[0][:, 0], inpRanges[0][:, 1] - x0, (inpRanges[0][:, 1] - inpRanges[0][:, 0]) / 10])
        ...
        if local_search == 'gradient':
            while local_counter <= max_loc_iter and TR_size > eps_tr * min(
                    (inpRanges[0][:, 1] - inpRanges[0][:, 0]).flatten()) and dimOK4budget:
                start = sim_count
