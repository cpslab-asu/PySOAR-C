...
####### v5 ########## changed initialization of m, store global and local samples separately
def PySOAR(inpRanges, prob, repNum, local_search):
    ...
    while sim_count < nSamples:
        ####uncomment for official testing####
        # if global_counter >= 5:
        #     break
        ######################################
        ...
        all_local_x = x0
        all_local_y = f0
        # initialize m: the number of points that is already in TR, here m=1 comes from the x0 of the global GP
        # # print('%%%%%%%%%%%%%%%%%%%%%%%%%%%', all_local_x.reshape(-1, 1), TR.T[:, 0],  np.all(all_local_x.reshape(-1, 1) >= TR.T[:, 0].reshape(-1, 1)))
        # # print('$$$$$$$$$$$$$$$$$$$$$$$$$$$', all_local_x.reshape(-1, 1), TR.T[:, 1], np.all(all_local_x.reshape(-1, 1) <= TR.T[:, 1].reshape(-1, 1)))
        # # print('all TF', np.all(np.vstack([True, True]) , axis=0))
        # local_in_TR_idx = np.all(np.vstack([np.all(all_local_x.reshape(-1, 1) >= TR.T[:, 0].reshape(-1, 1)),
        #                                     np.all(all_local_x.reshape(-1, 1) <= TR.T[:, 1].reshape(-1, 1))]), axis=0)
        # print('%%%%%%%%%%%%%%%%%%%%%%%%%%%', all_local_x.shape, TR[0, :].shape,  np.all(all_local_x >= TR[0, :]))
        # print('$$$$$$$$$$$$$$$$$$$$$$$$$$$', all_local_x, TR[1, :], np.all(all_local_x <= TR[1, :]))
        local_in_TR_idx = np.all(np.vstack([np.all(all_local_x >= TR[0, :]),
                                            np.all(all_local_x <= TR[1, :])]), axis=0)
        ...
        while TR_size > eps_tr * min((inpRanges[0][:, 1] - inpRanges[0][:,0]).flatten()) and sim_count+n_0-m < nSamples and dimOK4budget:
            print('TR',inpRanges[0][:, 1], inpRanges[0][:,0])
            ####uncomment for official testing####
            # if local_counter > 5:
            #     break
            ######################################
            ...
            # local_in_TR_idx = np.all(np.vstack([np.all(all_local_x >= TR.T[:,0].reshape(-1, 1), axis=1), np.all(all_local_x <= TR.T[:,1] , axis=1)]), axis=0)
            # print('%%%%%%%%%%%%%%%%%%%%%%%%%%%', all_local_x, TR[0, :].reshape(-1, 1).T, TR[1, :].reshape(-1, 1).T,  np.all(all_local_x >= TR[0, :].reshape(-1, 1).T, axis = 1))
            # print('all TF', np.vstack([np.all(all_local_x >= TR[0, :].reshape(-1, 1).T, axis = 1),
            #                                     np.all(all_local_x >= TR[1, :].reshape(-1, 1).T, axis = 1)]))
        ...
        print(sim_count)
    print(history['samples'].shape, history['rob'].shape, history['f_star'].shape, history['global_samples'].shape, history['local_samples'].shape)
    print('SOAR_Taliro: Samples Exhausted!')
    run['nTests'] = nSamples
    print(traj)
    return run, history
