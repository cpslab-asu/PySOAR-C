...
def _initialize(soar_options:SoarOptions, test_fn, rng: np.random.Generator) -> InitializationPhase:
    ...
    initial_samples = lhs_sampling(soar_options.n_0, soar_options.inpRanges, soar_options.tf_dim, rng)
    initial_sample_distances = _evaluate_samples(initial_samples, test_fn, soar_options.behavior)

    initial_points = InitializationPhase(
        samples=initial_samples, 
        costs=initial_sample_distances
    )
...
def soarc(
    ...
    result = Result(_initialize(soar_options, fn, rng))
