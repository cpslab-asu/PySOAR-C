...
with open("TEST123/NLF_1_budget_500_1_reps/NLF_1_budget_500_1_reps_seed_1234567.pkl", "rb") as f:
...
for i in np.unique(distinction):
    ...
    if subset_1.shape[0] != 0:
        all_points = np.stack(subset_1[:,1])
        ax.plot(all_points[:,0], all_points[:,1], ".", markersize = 1)
        ax.plot(all_points[0,0], all_points[0,1], "*", markersize = 4)
plt.show()
